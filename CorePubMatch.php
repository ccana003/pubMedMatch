...
class CorePubMatch extends AbstractExternalModule
{
    ...
    public function redcap_every_page_top($project_id = null): void
    {
        ...
        if ($this->isSurveyPage()) {
            $this->renderSurveyMatchUi((int) $project_id);
            return;
        }
...
HTML;
    ...
    /**
     * Inject survey matching UI shell and script.
     */
    private function renderSurveyMatchUi(int $projectId): void
    {
        $identifier = trim((string) ($_GET['core_pubmatch_identifier'] ?? ''));
        if ($identifier === '') {
            return;
    ...
    private function getSurveyMatches(int $projectId, string $identifier): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($recordId === $identifier || $investigatorEmail === $identifierLower || $investigatorName === $identifierLower) {
                $matchableRecordIds[$recordId] = true;
            }
        }
        ...
        foreach ($rows as $row) {
            ...
            $recordId = trim((string) ($row['record_id'] ?? ''));
            if (!isset($matchableRecordIds[$recordId])) {
                continue;
            }
            ...
            $cards[] = [
                ...
                'status' => trim((string) ($row['status'] ?? '0')),
            ];
        }
